...
class OrderController extends Controller
{
    ...
    protected function getFactoryPOPage()
    {
        ...
        return view('Itemrequest.porequest_page', compact('po_lists', 'currentDate'));
    }

    protected function searchFactoryPoList(Request $request)
    {

        $factory = FactoryPO::whereBetween('po_date', [$request->from, $request->to])->get();

        return response()->json($factory);
    }

    protected function factoryPoExport(Request $request, $from, $to, $type)
    {
        return $this->excel->download(new FactoryPoListExport($from, $to, $type), 'Factory_PO_List.xlsx');
    }
    ...
    public function saveFactoryOrder(Request $request, $id)
    {
        ...
        $factoryOrder->finish_status = $request->finish_status;
        $factoryOrder->save();
        return redirect()->back();
    }
    ...
    public function destoryFactoryItem($id){
