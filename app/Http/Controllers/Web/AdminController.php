...
class AdminController extends Controller {
 ...
 protected function getReceivablePayable(Request $request){
     ...
     $date = new DateTime('Asia/Yangon');
        ...
        $orderCustomer = OrderCustomer::where('name','LIKE','%showroom%')->get();
        $orderCustomers=[];
        foreach($orderCustomer as $oc){
            array_push($orderCustomers,$oc->id);
        }
    //  $orders = Order::whereBetween('order_date',[$current_Date,$current_Date])->whereNotIn('customer_id',[6,53,54,100,101,102,138,139,141,173,174,196,213,218,248,339,375,376,410])->get();
        $orders = Order::whereBetween('order_date',[$current_Date,$current_Date])->whereNotIn('customer_id',$orderCustomers)->get();
     $unpaid_orders = [];
     if($orders != null){
                    ...
                    if($request->adjustment == 2){
                       ...
                       $balance_qty = ($factory_item->instock_qty + $qty[$count]);
                        ...
                        if($factory_item->category_id === 9){
                            ...
                            {
                                array_push($fcount,$f_count->factory_item_id);
                            }
                                if(in_array($factory_item->id,$fcount)){
                                    $fa_count =FabricCount::where('count_date',$request->purchase_date)->where('factory_item_id',$factory_item->id)->first();
                                    $fa_count->in_stock += $qty[$count];
                                    $fa_count->close_stock += $qty[$count];

                                    $fa_count->save();
                                }
                                if(!in_array($factory_item->id,$fcount)){

                                    $fabric_before_entry = FabricEntryItem::where('factory_item_id',$factory_item->id)->first();
...
//        return $request;
    ...
    public function saleCustomerExport()
    {
        return Excel::download(new SaleCustomersExport, 'saleCustomers.xlsx');
    }

    public function orderCustomerExport()
    {
        return Excel::download(new OrderCustomersExport, 'orderCustomers.xlsx');
    }
    ...
    {
        $data = FacebookLink::all();
        return view('Admin.facebook', [
            'facebook_links'=> $data
        ]);
    }

    public function facebook_create(Request $request)
    {
        // dd($request->file('facebook_photo'));
        $validator = validator(request()->all(),[
            "facebook_title"=>'required',
            "facebook_photo"=>'required',
        ...
        // $name = $request->file('facebook_photo')->getClientOriginalName();
        // $request->file('facebook_photo')->storeAs('public/facebook_images/', $name);

         $user_code = $request->session()->get('user')->user_code;

//

        $file = $request->file('facebook_photo');
        $fileName = uniqid().'_'.$file->getClientOriginalName();
        $file->move(public_path().'/facebook_images/', $fileName);



        // $image = $request->file('facebook_photo');

        // $photo_path = $image->getClientOriginalName();

        // $image->move(public_path() .'/photo/facebook/', $photo_path);

        $facebook=FacebookLink::create([
            'facebook_title' => $request->facebook_title,
            'facebook_description' => $request->facebook_description,
            'facebook_photo' => $fileName,
    ...
    public function facebook_store(Request $request)
	{
        ...
        try {
            ...
            alert()->error('Something Wrong! When Creating Facebook Link.');
    ...
    public function facebook_update($id, Request $request)
	{
		try {
        	...
        	$facebook = FacebookLink::findOrFail($id);

   		} catch (\Exception $e) {

        	alert()->error("Facebook Link Not Found!")->persistent("Close!");

            return redirect()->back();

    	}
        if($request->hasfile('facebook_photo')){
            $file = $request->file('facebook_photo');
            $fileName = uniqid().'_'.$file->getClientOriginalName();
            $file->move(public_path().'/facebook_images/', $fileName);
        }else{
            $fileName = $facebook->facebook_photo;
        }



        $facebook->facebook_title = $request->facebook_title;

        $facebook->facebook_description = $request->facebook_description;

        $facebook->facebook_photo = $fileName;

        $facebook->facebook_link = $request->facebook_link;
    ...
    public function facebook_delete(Request $request)
	{


		$id = $request->facebook_id;

        $facebook = FacebookLink::find($id);
        $photoName= $facebook->facebook_photo;

            if(File::exists(public_path().'/facebook_images/'.$photoName)){
                File::delete(public_path().'/facebook_images/'.$photoName);
            }
            $facebook->delete();
    ...
    {
        $data = YoutubeLink::all();
        return view('Admin.youtube', [
            'youtube_links'=> $data
        ]);
    }

    public function youtube_create(Request $request)
    {
        $validator = validator(request()->all(),[
    ...
    public function youtube_store(Request $request)
	{
        ...
        try {
           ...
           // $category->save();
            ...
            alert()->error('Something Wrong! When Creating Youtube Link.');
    ...
    public function youtube_update($id, Request $request)
	{
		try {
        	...
        	alert()->error("Youtube Link Not Found!")->persistent("Close!");
    ...
    public function promotionphoto_index()
    {
        $data = PromotionPhoto::all();
        return view('Admin.promotionphoto', [
            'promotionphotos'=> $data
        ]);
    }

    public function promotionphoto_create(Request $request)
    {
        // $validator = validator(request()->all(),[
        //     "promotionphoto_title"=>'required',
        //     "promotionphoto_photo"=>'required',
        // ]);
        ...
        // $file = $request->file('promotionphoto_photo');
        // $fileName = uniqid().'_'.$file->getClientOriginalName();
        // $file->move(public_path().'/promotion_images/', $fileName);

        // $promotionphoto=PromotionPhoto::create([

        //     'promotionphoto_title' => $request->promotionphoto_title,
        //     'promotionphoto_photo' => $fileName,
...
//
    ...
    public function promotionphoto_store(Request $request)
	{
		$validator = Validator::make($request->all(), [
            'promotionphoto_title' => 'required',
            'promotionphoto_photo' => 'required',
        ...
        try {
            ...
            $promotionphoto = PromotionPhoto::create([
                'promotionphoto_title' => $request->promotionphoto_title,
                'promotionphoto_photo' => $request->promotionphoto_photo,
                'created_by' => $user_code,
            ]);
            ...
            alert()->error('Something Wrong! When Creating Promotion Photo.');
        ...
        return redirect()->route('promotionphoto');
	}

    public function promotionphoto_update($id, Request $request)
	{
		try {

        	$promotionphoto = PromotionPhoto::findOrFail($id);

   		} catch (\Exception $e) {

        	alert()->error("Promotion Photo Not Found!")->persistent("Close!");

            return redirect()->back();

    	}

        if($request->hasfile('promotionphoto_photo')){
            $file = $request->file('promotionphoto_photo');
            $fileName = uniqid().'_'.$file->getClientOriginalName();
            $file->move(public_path().'/promotion_images/', $fileName);
        }else{
            $fileName = $request->promotionphoto_photo;
        }

        $promotionphoto->promotionphoto_title = $request->promotionphoto_title;

        $promotionphoto->promotionphoto_photo = $fileName;
    ...
    public function promotionphoto_delete(Request $request)
	{
		$id = $request->promotionphoto_id;

        $promotionphoto = PromotionPhoto::find($id);

        $promotionphoto->delete();
