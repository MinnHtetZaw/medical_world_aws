...
use App\Exports\ItemAdjustListExport;
...
class StockController extends Controller
{
    ...
    {
        $item = Item::findOrFail($request->unit_id);
        $item->hot_sale_flag = $request->chek_value;
        $item->save();
    ...
    public function purchaseUpdateAjax(Request $request)
    {
        ...
        if ($purchase->purchase_type == 1) {
            ...
            if ($purchase->adjustment_flag == 1) {
                try {

                    // $purchase_factory_item=DB::table('factory_item_purchase')->where('factory_item_id',$request->unit_id)->where('purchase_id',$request->purchase_id)->first();
                    // if($purchase_factory_item->quantity == $request->new_qty){
                    // }

                    $update_stock = FactoryItem::find($request->unit_id);
                    if ($request->unit_id == $update_stock->id) {

                        $update_stock->instock_qty += $request->new_qty;
                        $update_stock->save();
                    }
                } catch (Exception $e) {
                    return response()->json(0);
    ...
    protected function saveFabricCount(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                $fabric = FabricEntryItem::where('factory_item_id', $item->id)->first();
                $fabric->instock_qty = $item->close_stock;
                $fabric->save();
    ...
    protected function saveArriveFabric(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $fabric_update_entry = FabricEntryItem::where('factory_item_id', $item->id)->first();
                if (in_array($item->id, $item_ids)) {
                    ...
                    $fabricCount->open_stock = $fabric_update_entry->instock_qty;
                    $fabricCount->in_stock = $item->arrive_qty;
                    $fabricCount->out_stock = 0;
                    $fabricCount->close_stock = $item->arrive_qty + $fabric_update_entry->instock_qty;
                    $fabricCount->remark = $arrive_remark;
                    $fabricCount->save();
                }
                $fabric_update_entry->instock_qty += $item->arrive_qty;
                ...
                $factory_fabric_save_date->remark = $arrive_remark;
                $factory_fabric_save_date->arrive_date = $arrive_format_date;
                $factory_fabric_save_date->save();

                // if($purchase->factory_item->pivot->factory_item_id === $itme->id){
                //     $remain =$purchase->factory_item()->pivot->
                // }
                // $remain = $purchase->factory_item()->pivot->remaining_amount;
                // $remain = $remain - $item->arrive_qty;
    ...
    protected function get_fabricentry_item(Request $request)
    {
        ...
        if ($request->entry_date != null) {
            ...
            $fabric_entry_item->status = 1;
        } elseif ($request->entry_date === null) {
            $fabric_entry_item = FabricEntryItem::where('factory_item_id', $request->item_id)->first();
            $fabric_entry_item->status = 0;
        }
    ...
    protected function exportitemadjustLists(Request $request, $from, $to)
    {

        return $this->excel->download(new ItemAdjustListExport($from, $to), 'item_adjust_list.xlsx');
    }
