...
class EcommerceOrderApiController extends ApiBaseController
{
   ...
   public function searchitem(Request $request){
     $item = Item::where('item_name','like','%'.$request->item.'%')->where('instock',1)->get();
   ...
   public function typegender($name,$gen){
    ...
    $unit = CountingUnit::where('design_id',$design->id)->where('gender_id',$gender->id)->get('fabric_id');
   ...
   public function typefabric($name,$gen,$fabric){
    ...
    $count = collect($count)->unique();
        ...
        $count = ['ar7','ar16', 'ar18', 'ar20','ar21','ar26','ar32','ar36','ar43', 'ar44','ar48','ar57',
        'ar64','ar66', 'ar67', 'ar70','ar75', 'ar77', 'ar79','ar81', 'ar82','ar88', 'ar90','ar96', 'ar98',
        'ar101','ar106'];
        // $count = Colour::where('fabric_id',1)->get();
        // dd(count($count));
    }
   ...
   public function typefabric1($name,$gen,$fabric){
    $design = Design::where('design_name',$name)->first();
    $gender = Gender::where('gender_name',$gen)->first();
    $fabric = Fabric::where('fabric_name',$fabric)->first();
   ...
   public function typecolour($name,$gen,$fabric,$colour){
    ...
    $count = collect($count)->unique();
        ...
        $unit = Size::take(16)->get();
        $count = [];
        foreach($unit as $u){
            array_push($count,$u->size_name);
        }
    }
   ...
   public function invoice_mail(Request $request)
    ...
    public function colourapi($fabric,$flag){

        $fabric_data = Fabric::where('fabric_name',$fabric)->first();

        if($flag == 1){
            $color = Colour::select('id','colour_name','colour_description','color_hex_code')->where('preorder_flag',$flag)->get();

        }else{
            $color = Colour::select('id','colour_name','colour_description')->where('fabric_id',$fabric_data->id)->get();
